...
export const usersPageActions = {
    followAC: (userId: number) =>
        ({ type: 's_n/users/FOLLOW', userId } as const),
    unfollowAC: (userId: number) =>
        ({ type: 's_n/users/UNFOLLOW', userId } as const),
    setUsersAC: (users: Array<ResponseUserType>) =>
        ({ type: 's_n/users/SET_USERS', users } as const),
    setCurrentPageAC: (page: number) =>
        ({ type: 's_n/users/SET_CURRENT_PAGE', page } as const),
    setTotalUsersCountAC: (usersCount: number) =>
        ({ type: 's_n/users/SET_TOTAL_USERS_COUNT', usersCount } as const),
    toggleIsFetchingAC: (isFetching: boolean) =>
        ({ type: 's_n/users/TOGGLE_IS_FETCHING', isFetching } as const),
...
export const unfollow = (userId: number): UsersThunkType => async (
    dispatch
) => {
    await _followUnfollowFlow(
        dispatch,
