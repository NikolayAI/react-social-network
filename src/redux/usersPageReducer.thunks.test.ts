...
test('success follow thunk', async () => {
    ...
    expect(dispatchMock).toHaveBeenNthCalledWith(1, usersPageActions.toggleFollowingProgressAC(true, 1))
    expect(dispatchMock).toHaveBeenNthCalledWith(2, usersPageActions.followAC(1))
    expect(dispatchMock).toHaveBeenNthCalledWith(3, usersPageActions.toggleFollowingProgressAC(false, 1))
})
