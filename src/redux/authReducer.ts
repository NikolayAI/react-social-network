...
const authActions = {
    setAuthUserDataAC: (userId: number | null, login: string | null, email: string | null, isAuth: boolean) => (
        {type: 's_n/auth/SET_AUTH_USER_DATA', payload: {userId, login, email, isAuth}} as const),
    getCaptchaUrlSuccessAC: (captchaUrl: string) => ({type: 's_n/auth/SET_CAPTCHA_URL_SUCCESS', payload: {captchaUrl}} as const),
}
...
export const getCaptchaUrl = (): AuthThunkType => async (dispatch: ThunkDispatch<StateAuthType, {}, ActionsAuthTypes>) => {
...
export const login = (email: string, password: string, rememberMe: boolean, captcha: string | null): AuthThunkType =>
    async (dispatch) => {
